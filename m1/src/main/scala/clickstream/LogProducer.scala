...
object LogProducer extends App {
  ...
  //  val fw = new FileWriter(filePath, true)
    ...
    for (iteration <- 1 to wlc.numFile) {
  ...
  //  val outFile = FileUtils.getFile(s"${destPath}data_$timestamp")
  //  println(s"Moving produced data to ${outFile}")
  //  FileUtils.moveFile(FileUtils.getFile(filePath), outFile)
    val sleeping = 2000
    println(s"Sleeping for ${sleeping} m-secs")
  }
